...
export class PostgresEventStore implements EventStore {
  ...
  async getEvents(aggregateId: string): Promise<BaseEvent[]> {
    ...
    try {
      const result = await client.query(
        `SELECT event_type as type, event_data as data, version, timestamp 
         FROM events 
  ...
  async getAllEvents(): Promise<Array<BaseEvent & { aggregateId: string; id: string; version: number }>> {
    const client = await this.pool.connect();
  ...
  async getEventsByType(eventType: string): Promise<Array<BaseEvent & { aggregateId: string; id: string; version: number }>> {
    const client = await this.pool.connect();
  ...
  async saveEvent(event: BaseEvent): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('duplicate key')) {
        throw new Error(`Concurrency conflict: version ${event.version} already exists for aggregate ${event.aggregateId}`);
      }
      throw error;
    } finally {
      client.release();
  ...
  async getEvents(aggregateId: string): Promise<BaseEvent[]> {
    ...
    try {
      const result = await client.query(
        `SELECT aggregate_id, event_type, version, timestamp, data 
         FROM events 
         WHERE aggregate_id = $1 
         ORDER BY version ASC`,
        [aggregateId]
